import argparse
import torch
import torchvision.transforms as transforms
from torch.utils.data import DataLoader
from torch.autograd import Variable
import os
from tools.utils import get_config
import datetime
from tools.datasets import ImageDataset, ValDataset
from Model.generator import Gan_AttU_Net,UNetGenerator
from Model.discriminator import Discriminator_att,Discriminator
from tools.utils import Resize
import losses
from Model.reg import Reg1
from tools.transformer import Transformer_2D
from Segmentation.SegLoss import AdaptiveLossWeight
from evaluation import MAE,SSIM
from draw_plot import draw_loss
# from torch.utils.tensorboard import SummaryWriter

os.environ["CUDA_VISIBLE_DEVICES"] = "0"

chan = [0, 0]
diameter = 38
adp_loss_weight = AdaptiveLossWeight()
isPrintRes = False
isSaveRes = False

class Gan_Trainer():
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.exp_name_title = 'G:unetG；D：D-att'

        # self.writer = SummaryWriter(log_dir=config['save_root'])

        # attunet -Gan and special D
        self.netG_A2B = Gan_AttU_Net(config['input_nc'], config['output_nc'],type='instanceNorm',scale_factor=1).cuda()
        # self.netG_A2B = UNetGenerator(config['input_nc'], config['output_nc']).cuda()

        self.netD_B = Discriminator_att(config['input_nc']).cuda()
        # self.netD_B = Discriminator(config['input_nc']).cuda()
        self.optimizer_D_B = torch.optim.Adam(self.netD_B.parameters(), lr=config['lr'], betas=(0.5, 0.999))
        # 是否配准

        # self.R_A = Reg1(config['size'], config['size'], config['input_nc'], config['input_nc']).cuda()
        # self.spatial_transform = Transformer_2D().cuda()
        # self.optimizer_R_A = torch.optim.Adam(self.R_A.parameters(), lr=config['lr'], betas=(0.5, 0.999))

        self.optimizer_G = torch.optim.Adam(self.netG_A2B.parameters(), lr=config['lr'], betas=(0.5, 0.999))

        # Lossess
        # self.MSE_loss = losses.mse_loss()
        self.BCE_loss = losses.bce_loss()
        self.L1_loss = losses.n1_loss()
        self.ncc = losses.NCC(win=20, eps=1e-3)
        # self.Seg_loss = segmentation_loss
        self.grad_loss = losses.smooothing_loss
        # Inputs & targets memory allocation
        Tensor = torch.cuda.FloatTensor if config['cuda'] else torch.Tensor
        self.input_A = Tensor(config['batchSize'], config['input_nc'], config['size'], config['size'])
        self.input_B = Tensor(config['batchSize'], config['output_nc'], config['size'], config['size'])

        self.input_A_test = Tensor(config['batchSize_val'], config['input_nc'], config['val_size'], config['val_size'])
        self.input_B_test = Tensor(config['batchSize_val'], config['output_nc'], config['val_size'], config['val_size'])

        # Dataset loader
        train_transforms = [
            transforms.ToTensor(),
            # 👇 在这里加数据增强（医学图像安全版，不破坏结构）
            # transforms.RandomHorizontalFlip(p=0.5),  # 随机水平翻转 50% 概率
            # transforms.RandomVerticalFlip(p=0.5)  # 随机垂直翻转 50% 概率
            Resize(size_tuple=(config['size'], config['size']))
        ]

        val_transforms = [transforms.ToTensor(),
                          Resize(size_tuple=(config['val_size'], config['val_size']))]
        self.dataloader = DataLoader(
            ImageDataset(config['dataRoot'], transforms_=train_transforms),
            batch_size=config['batchSize'], shuffle=True, num_workers=config['n_cpu'], drop_last=True)

        self.val_data = DataLoader(ValDataset(config['val_dataRoot'], transforms_=val_transforms),
                                   batch_size=1, shuffle=False, num_workers=config['n_cpu'], drop_last=True)

    def train(self, cfg_content):
        total_loss_mean = []
        Reg_loss_mean = []  # 配准损失 = dvf loss + tran loss
        dvf_loss_mean = []  # 形变矢量场的loss
        tran_loss_mean = []  # 形变修正后 于 targe图像的loss
        adv_loss_mean = []
        loss_dis_mean = []
        loss_adv_Seg = []
        total_datasets = len(self.dataloader) * self.config['batchSize']
        print(f'total train dataset = {total_datasets}')
        epoch_loops = self.config['n_epochs']
        exp_res_dir = os.path.join(self.config["save_root"], cfg_content.split(':')[1])
        if not os.path.exists(exp_res_dir):
            os.makedirs(exp_res_dir)
        with open(f"{exp_res_dir}/cfg_readme.txt", "a", encoding="utf-8") as file:
            file.write(cfg_content)

        ###### Training ######
        print('time',datetime.datetime.now(),'exp: ', self.exp_name_title)
        for epoch in range(self.config['epoch'], self.config['n_epochs']):
            print(f'----------------epoch={epoch}/{epoch_loops - 1}----------------')
            loss1, loss2, loss3, loss4, lossD, loss_Seg = 0, 0, 0, 0, 0, 0
            loss_l1 = 0
            for i, batch in enumerate(self.dataloader):
                # Set model input
                real_A = Variable(self.input_A.copy_(batch['A']))
                real_B = Variable(self.input_B.copy_(batch['B']))

                # self.optimizer_R_A.zero_grad()
                self.optimizer_G.zero_grad()
                #### regist sys loss
                fake_B = self.netG_A2B(real_A)
                # 计算dvf 即 Trans
                # Trans = self.R_A(fake_B, real_B)
                # 精配准虚拟染色 即 SysRegist  空间转化
                # fake_B_reg = self.spatial_transform(fake_B, Trans)

                L1_Transform = self.config['Corr_lamda'] * self.L1_loss(fake_B, real_B)  ###SR

                # 计算fake img 的  D损失
                pred_fake0 = self.netD_B(fake_B)

                adv_loss = self.config['Adv_lamda'] * self.BCE_loss(pred_fake0, torch.ones_like(pred_fake0).cuda())

                ####smooth loss  这里是dvf的损失
                # SM_loss = self.config['Smooth_lamda'] * self.grad_loss(Trans)


                # 总损失
                total_loss = adv_loss + L1_Transform
                # toal_loss = adv_loss + SR_loss + SM_loss

                total_loss.backward()
                # self.optimizer_R_A.step()
                self.optimizer_G.step()
                self.optimizer_D_B.zero_grad()

                # for name, param in self.R_A.named_parameters():
                #     # 记录参数的均值（看整体变化趋势）
                #     self.writer.add_scalar(f"Params/{name}/mean", param.mean(), epoch)
                #     # 记录参数的标准差（看离散程度/变化幅度）
                #     self.writer.add_scalar(f"Params/{name}/std", param.std(), epoch)
                #     # 记录参数的直方图（看分布变化）
                #     self.writer.add_histogram(f"Params/{name}/hist", param, epoch)

                with torch.no_grad():
                    fake_B = self.netG_A2B(real_A)
                pred_fake0 = self.netD_B(fake_B)
                pred_real = self.netD_B(real_B)
                loss_D_B = self.config['Adv_lamda'] * (self.BCE_loss(pred_fake0, torch.zeros_like(pred_fake0).cuda()) +
                                                       self.BCE_loss(pred_real, torch.ones_like(pred_real).cuda())) / 2
                # 更新 判别器 D
                loss_D_B.backward()
                self.optimizer_D_B.step()

                lossD += loss_D_B.item()
                loss1 += total_loss.item()   # total loss
                # loss2 += (SM_loss.item() + SR_loss.item())  # Reg loss 整体的配准损失
                # loss3 += SM_loss.item()  # dvf loss///////
                loss4 += adv_loss.item()  # adv loss
                loss_l1 += L1_Transform.item()
#                 loss_Seg += loss_seg.item() / total_datasets
                if i % 10 == 0:
                    # print(f'total loss:{toal_loss} dvf_loss:{SM_loss} adv_loss:{adv_loss} L1_loss:{SR_loss}')
                    print(f'total loss:{total_loss}  adv_loss:{adv_loss} L1_loss:{L1_Transform}')

            total_loss_mean.append(loss1 / total_datasets) # 总损失
            Reg_loss_mean.append(loss2  / total_datasets) # 配准损失 = 形变场 + L1/NCC
            dvf_loss_mean.append(loss3  / total_datasets) # 形变场损失
            tran_loss_mean.append(loss_l1/ total_datasets)  # 图像的转化损失
            adv_loss_mean.append(loss4  / total_datasets) # 生成对抗损失  生成器
            loss_dis_mean.append(lossD / total_datasets) # 判别器损失

            # Save models checkpoints
            if epoch >= (self.config['n_epochs'] / 3) and epoch % 2 == 0:
                torch.save(self.netG_A2B.state_dict(), exp_res_dir + f'/epoch={epoch}_netG_A2B.pth')

            #############val###############
            with torch.no_grad():
                SSIM_total = 0
                num = 0
                for i, batch in enumerate(self.val_data):
                    real_A = Variable(self.input_A_test.copy_(batch['A']))
                    real_B = Variable(self.input_B_test.copy_(batch['B'])).detach().cpu().numpy().squeeze()
                    fake_B = self.netG_A2B(real_A).detach().cpu().numpy().squeeze()
                    # ssim = MAE(fake_B, real_B)
                    ssim = SSIM(fake_B, real_B)
                    SSIM_total += ssim
                    num += 1

                print('Val SSIM:', SSIM_total / num)

        draw_loss(self.exp_name_title,total_loss_mean,'total_loss',exp_res_dir)
        draw_loss(self.exp_name_title,Reg_loss_mean, 'Registration loss',exp_res_dir)
        draw_loss(self.exp_name_title,dvf_loss_mean, 'dvf_smooth_loss',exp_res_dir)
        draw_loss(self.exp_name_title,tran_loss_mean, 'transformation loss',exp_res_dir)
        draw_loss(self.exp_name_title,adv_loss_mean, 'G-loss',exp_res_dir)
        draw_loss(self.exp_name_title,loss_dis_mean, 'D-loss',exp_res_dir)
        # draw_loss(self.exp_name_title,loss_adv_Seg, 'Adversary Segmentation',exp_res_dir)


def log_metrics(file_path, metrics):
    """将指标追加到日志文件"""
    with open(file_path, 'a', encoding='utf-8') as f:  # 'a' 表示追加模式
        f.write(f"{metrics}\n")  # 直接写入字典或字符串

if '__main__' == __name__:
    cfg = ('multi-organ:Liver_epoch=150_weight')
    parser = argparse.ArgumentParser()
    parser.add_argument('--config', type=str, default='Yaml/Gan_baseline_28.yaml', help='Path to the config file.')
    opts = parser.parse_args()
    config = get_config(opts.config)

    Trainer = Gan_Trainer(config=config)
    sdir = Trainer.config['save_root']
    exp_name = Trainer.exp_name_title
    logtxt = os.path.join(sdir,'config.txt')

    Trainer.train(cfg_content=cfg)